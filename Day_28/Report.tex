...
and paired with a self-contained HTML/CSS/JS frontend served from the same container --
deployment-ready for Hugging Face Spaces (Docker SDK), whose free CPU tier's 16GB RAM
comfortably fits this task's three-model footprint, unlike Render/Railway's $\sim$512MB--1GB
free tiers.
...
directly, D2), comfortably inside HF Spaces' free 16GB tier.
...
\textbf{C3 -- cloud deployment.} Target: Hugging Face Spaces (Docker SDK) -- chosen over
Render/Railway specifically because its free CPU tier offers 16GB RAM against their
$\sim$512MB--1GB, comfortably fitting this task's 695MB three-model footprint without needing
to drop a model. A GitHub Actions workflow (\texttt{.github/workflows/deploy-hf-spaces.yml})
mirrors this directory to a Space's git remote on every push to \texttt{main}, which triggers
Spaces' own automatic rebuild/redeploy -- the CI/CD pipeline the brief asks for. Actually
pushing this live requires an account-specific Hugging Face token, which was left as the final,
credential-gated step rather than attempted without the necessary account access (see
README.md's step-by-step deployment instructions).
...
files work unmodified against localhost, a local container, or the live Spaces URL. Verified
