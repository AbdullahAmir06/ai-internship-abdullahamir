...
\begin{lstlisting}[style=py, caption={Indexing, slicing, reshaping, and element by element maths.}]
...
The \texttt{axis} argument is worth pausing on: \texttt{axis=0} runs down the rows (so you get one result per column) and \texttt{axis=1} runs across the columns (one result per row). I mix these up less once I picture the axis as the one that ``collapses''.
...
\begin{lstlisting}[style=py, caption={Broadcasting a 1D array across a 2D array.}]
...
NumPy also covers the basic linear algebra that turns up all the time in machine learning. Dot products, matrix multiplication, transposes, and inverses are all one call away.
...
A couple of things I keep in mind here: \texttt{*} multiplies element by element, while \texttt{@} (or \texttt{np.dot}) does true matrix multiplication, which is an easy trap to fall into. And the inverse only exists for a square matrix whose determinant isn't zero, otherwise \texttt{np.linalg.inv} raises an error.
...
Pandas builds on top of NumPy and gives you two main objects. A \texttt{Series} is a labelled array with one dimension, basically a single column with an index. A \texttt{DataFrame} is the two dimensional table you get by putting several of those columns side by side, which is how most real data actually looks.
...
Once the data is in a DataFrame, most of the work is pulling out the parts you care about. I use \texttt{loc} to select by label and \texttt{iloc} to select by position, and boolean masks to filter by a condition.
...
The \texttt{groupby} pattern is the one I use most for summarising. It follows a simple split, apply, combine idea: split the rows into groups, apply an aggregation to each group, then combine the results back into a table.
...
\begin{itemize}[leftmargin=1.4em]
    ...
    \item \textbf{Fare tracks class closely.} First class passengers paid far more on average (around \pounds84) than third class ones (around \pounds14), which lines up with them being berthed closer to the lifeboats.
    \item \textbf{Age mattered too.} Children had the best odds by age group (about 58\%), while seniors had the worst (about 23\%).
    \item Combining class and age showed the effects stacking up: a child in first or second class was very likely to survive, while an older third class passenger was among the least likely.
\end{itemize}

The full project, including the cleaning script, the analysis code, and a \texttt{README.md} that describes the dataset, the cleaning process, the analysis, and these findings, is uploaded to my GitHub repository for the internship: \url{https://github.com/AbdullahAmir06/ai-internship-abdullahamir}.

% ======================================================================
\section*{Conclusion}
\addcontentsline{toc}{section}{Conclusion}
This task tied together the numerical and data handling side of Python. NumPy gave me the foundation, with arrays, broadcasting, and vectorization that runs far faster than loops written by hand, while Pandas built on top of it with Series, DataFrames, and the filtering, grouping, and merging tools that make real datasets manageable. Pulling it all together in the Titanic mini project showed how the pieces fit: load messy data, clean it carefully, and then let \texttt{groupby} surface the story hidden inside it. These are the exact skills the rest of the AI and ML work in the internship will keep leaning on.
